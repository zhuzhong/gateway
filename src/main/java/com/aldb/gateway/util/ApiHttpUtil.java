...
public class ApiHttpUtil {
	...
	public static String HttpPost(String webUrl, Map paramMap) {
		...
		httpClient.getHttpConnectionManager().getParams().setConnectionTimeout(TIME_OUT_MILL_SEC);
		httpClient.getHttpConnectionManager().getParams().setSoTimeout(TIME_OUT_MILL_SEC);
	...
	public static String doPost(String url, String reqData, String contentType) {
		...
		client.getHttpConnectionManager().getParams().setConnectionTimeout(TIME_OUT_MILL_SEC);
		client.getHttpConnectionManager().getParams().setSoTimeout(TIME_OUT_MILL_SEC);
		HttpClientParams httpClientParams = new HttpClientParams();
		httpClientParams.setParameter(HttpMethodParams.RETRY_HANDLER, new DefaultHttpMethodRetryHandler(0, false));
		httpClientParams.setBooleanParameter(HttpConnectionParams.STALE_CONNECTION_CHECK, true);
